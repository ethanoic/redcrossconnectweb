...
if ($_SERVER['REQUEST_METHOD']=="POST")
{
	...
	if ($_POST["agreetoreceivealerts"]=="I-agree-to-receive-alerts-from-Singapore-Red-Cross")
	{
		$agreed_status = "yes";
	}
	...
	{
		$myFile = "1t4r7lmaafpdrb132-database.txt";
		$file = fopen($myFile, 'a') or die("can't open file");
		$stringData = date("d F, Y" ,time()) . " " . date("h:i A" ,time()) . "\t" . $_POST["emailaddress"] . "\t" . $_POST["donatedwilldonateatcampus"] . "\t" . $_POST["agreetoreceivealerts"] . "\r\n";
...
<div>
<input onClick="if(this.value=='Email')this.value='';" onBlur="if(this.value=='')this.value='Email';" type="text" name="emailaddress" 
<?php
if ($_SERVER['REQUEST_METHOD']=="POST")
{
	echo "value=\"" . $_POST["emailaddress"] . "\"";
}
else
{
	echo "value=\"" . "Email" . "\"";
...
if ($_SERVER['REQUEST_METHOD']=="POST")
{
	if ($_POST["donatedwilldonateatcampus"]=="I-have-donated-or-will-be-donating-blood-at-campus")
	{
		echo " checked ";
	}
}
